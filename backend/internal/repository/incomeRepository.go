...
func (r *IncomeRepository) GetAll(ctx context.Context) ([]models.Income, error) {
	query := `
		SELECT id, name, amount, type, incomeDate, status, notes, createdBy, createdAt, updatedAt
		FROM income
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&i.Status, &i.Notes, &i.CreatedBy, &i.CreatedAt, &i.UpdatedAt,
		)
		if err != nil {
			return nil, err
		}
...
func (r *IncomeRepository) Update(ctx context.Context, id int64, income *models.Income) (*models.Income, error) {
	query := `
		...
		WHERE id = $7
		RETURNING id, name, amount, type, incomeDate, status, notes, createdBy, createdAt, updatedAt
	`
	...
	err := r.db.QueryRow(ctx, query,
		...
		&income.Status, &income.Notes, &income.CreatedBy, &income.CreatedAt, &income.UpdatedAt,
	)
